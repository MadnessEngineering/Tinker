...
> **What this cost:** an earlier revision of this roadmap cited `browser/navigation.rs` as
> implementing per-tab history and counted its tests as coverage. Neither was true — the file had
> never compiled, and the real implementation is in `browser/tabs.rs`. Citing files is only
> useful if the citations are checked against what actually builds.
