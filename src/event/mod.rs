...
pub enum BrowserCommand {
    ...
    StartJSProfiling { script_url: String },
    StopJSProfiling,
    AddPerformanceMarker { name: String, metadata: Option<serde_json::Value> },
    AddPerformanceMeasure { name: String, start_mark: String, end_mark: String },
    // Console monitoring commands
    StartConsoleMonitoring,
    StopConsoleMonitoring,
    GetConsoleLogs { level: Option<String> },
    ClearConsoleLogs,
    SetConsoleFilter { level: Option<String> },
    ...
    LoadRecording { path: String },
    AddRecordingAssertion { expected_state: serde_json::Value, description: Option<String> },
    EnableRecordingSnapshots { interval_ms: u64 },
    StartPlayback,
    StopPlayback,
    PausePlayback,
    ResumePlayback,
    SeekPlayback { timestamp_ms: u64 },
    StepForward,
    StepBackward,
    SetPlaybackSpeed { speed: f32 },
    SetPlaybackLoop { enable: bool },
    GetPlaybackState,
}
...
pub enum BrowserEvent {
    ...
    PerformanceMetricsCollected { metrics: serde_json::Value },
    CoreWebVitalsUpdated { vitals: serde_json::Value },
    MemoryMetricsUpdated { metrics: serde_json::Value },
    JSProfilingStarted { profile_id: String },
    JSProfilingCompleted { profile: serde_json::Value },
    PerformanceMarkerAdded { name: String },
    PerformanceMeasureAdded { name: String, duration: f64 },
    // Console monitoring events
    ConsoleMessage { level: String, message: String, timestamp: u64, stack_trace: Option<String> },
    JavaScriptError { message: String, stack: String, filename: String, line: u32, column: u32 },
    ConsoleCleared,
    // Recording/Replay events
    RecordingStarted { name: String, start_url: String },
    RecordingStopped { name: String, event_count: usize, duration_ms: u64 },
    RecordingPaused,
    RecordingResumed,
    RecordingSaved { path: String },
    RecordingLoaded { path: String },
    PlaybackStarted,
    PlaybackStopped,
    PlaybackPaused,
    PlaybackResumed,
    PlaybackSeeked { timestamp_ms: u64 },
    PlaybackSpeedChanged { speed: f32 },
    PlaybackCompleted,
}
...
impl EventSystem {
    ...
    pub fn get_topic(&self, event: &BrowserEvent) -> &'static str {
        match event {
            ...
            BrowserEvent::PerformanceMetricsCollected { .. } => "browser/performance/metrics",
            BrowserEvent::CoreWebVitalsUpdated { .. } => "browser/performance/core-web-vitals",
            BrowserEvent::MemoryMetricsUpdated { .. } => "browser/performance/memory",
            BrowserEvent::JSProfilingStarted { .. } => "browser/performance/js-profiling/started",
            BrowserEvent::JSProfilingCompleted { .. } => "browser/performance/js-profiling/completed",
            BrowserEvent::PerformanceMarkerAdded { .. } => "browser/performance/marker",
            BrowserEvent::PerformanceMeasureAdded { .. } => "browser/performance/measure",
            // Console monitoring events
            BrowserEvent::ConsoleMessage { .. } => "browser/console/message",
            BrowserEvent::JavaScriptError { .. } => "browser/console/error",
            BrowserEvent::ConsoleCleared => "browser/console/cleared",
            // Recording/Replay events
            BrowserEvent::RecordingStarted { .. } => "browser/recording/started",
            BrowserEvent::RecordingStopped { .. } => "browser/recording/stopped",
            BrowserEvent::RecordingPaused => "browser/recording/paused",
            BrowserEvent::RecordingResumed => "browser/recording/resumed",
            BrowserEvent::RecordingSaved { .. } => "browser/recording/saved",
            BrowserEvent::RecordingLoaded { .. } => "browser/recording/loaded",
            BrowserEvent::PlaybackStarted => "browser/playback/started",
            BrowserEvent::PlaybackStopped => "browser/playback/stopped",
            BrowserEvent::PlaybackPaused => "browser/playback/paused",
            BrowserEvent::PlaybackResumed => "browser/playback/resumed",
            BrowserEvent::PlaybackSeeked { .. } => "browser/playback/seeked",
            BrowserEvent::PlaybackSpeedChanged { .. } => "browser/playback/speed",
            BrowserEvent::PlaybackCompleted => "browser/playback/completed",
        }
    }
